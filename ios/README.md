...
| Investments (allocation donut, asset-class / sector breakdown, per-holding drill-down) | `Views/InvestmentsView.swift` | `GET /api/allocation` |
...
`.github/workflows/ios.yml` runs two jobs on a GitHub-hosted **macOS runner**
on every push that touches `ios/`:

- **Build** — `xcodebuild build` against the iOS Simulator SDK (no code signing).
- **Unit Tests** — `xcodebuild test` runs the `KevFinTests` logic suite
  (`ios/KevFinTests/`) on the iOS Simulator: model decoding, the Forecast
  projection math, chart-range filtering, budget-category logic, and formatting.

This is the easiest way to confirm the project compiles *and* the pure logic is
correct without opening Xcode locally — check the **Actions** tab after pushing.
...
- Add stock look-through to **Investments** (the `/api/allocation` response also
  includes `byStock` and per-holding `accounts`).
